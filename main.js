...
async function handleListPokemon() {
    try {
        ...
        pokemons.forEach((p, index) => {
            console.log(`${index + 1}. [ID: ${p._id}] Nombre: ${p.nombre} | Tipo(s): ${p.tipo} | Peso: ${p.peso} kg | Altura ${p.altura} m`)
        })
        console.log("-------------------------------------")
    } catch (error){
...
async function handleUpdatePokemon(){
    ...
    try {
        ...
        if (!pokemon) {
            ...
            console.log(`Datos Actuales: [ID: ${pokemon._id}] Nombre: ${pokemon.nombre} | Tipo(s): ${pokemon.tipo} | Peso: ${pokemon.peso} kg | Altura ${pokemon.altura} m`)
            const nuevoTipo = readline.question("Ingrese el nuevo tipo para este Pokémon: ");

            const resultado = await pokemonDAO.update(pokemon._id, {tipo: nuevoTipo});
